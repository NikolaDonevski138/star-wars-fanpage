...
export const useSwapiShop = create((set, get) => ({
  ...
  setIncreaseOrderItem(elementId) {
    ...
    const selectedItemById = itemsListForChosenShopCategory.items.find(
      (el, index) => {
    ...
    const sumOfAllItems = itemsListForChosenShopCategory.items.reduce(
      (acc, el) => {
        return acc + el.sum;
      },
      0
    ...
    const mergedItems = [
      ...itemsListForChosenShopCategory.items.filter(
        (el, index) => el.productId !== elementId
      ),
      modifiedItem,
    ];

    const sortedItemsByProductIdAsc = mergedItems.sort((a, b) =>
  ...
  addToChart() {
    ...
    let totalItemsAddedToChart = Object.entries(
      updatedItemsListForShopCategory
    ).map((el) => {
    ...
    let totalOrderToChart = totalItemsAddedToChart.reduce(
  ...
  getAllItemsAddedToChart() {
    ...
    let itemsAddedToCart = Object.entries(itemsListForShopCategory).map(
      (el) => {
        if (!!el[el.length - 1]?.items === false) {
          return 0;
        }
  ...
  removeItemFromChart(elementId) {
    ...
    const selectedItemById = itemsAddedToCart.find(
      (el) => el.productId === elementId
    );
    ...
    const mergedItems = [
      ...itemsAddedToCart.filter((el, index) => el.productId !== elementId),
      modifiedItem,
    ];
    ...
    const totalNumInChart = itemsInChart.reduce(
      (previousValue, currentValue) => previousValue + currentValue
    );

    if (selectedItemById?.orderedItems === 1) {
      set({
        itemsAddedToCart: itemsAddedToCart.filter(
          (el, index) => el.productId !== elementId
        ),
      });
  ...
  removeAllItemsForChosenCategoryFromCart(elementId) {
    ...
    const updatedItem = itemsAddedToCart.filter(
      (el, index) => el.productId !== elementId
    );
    ...
    const totalNumInChart = itemsInChart.length
      ? itemsInChart.reduce(
          (previousValue, currentValue) => previousValue + currentValue
        )
      : 0;
  ...
  removeAllItemsFromCart() {
    set({ numberOfItemsAddedToChart: 0 });
    set({ itemsAddedToCart: [] });
  },

  setTotalPriceForPayments() {
    const { itemsAddedToCart } = get();

    const calcTotalSumOfItemsInCart = itemsAddedToCart
      .map((el) => {
        return el.sum;
      })
